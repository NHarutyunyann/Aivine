<template>
    <div>
        <div class="row mb-4 w-100 ml-auto mr-auto">
            <div class="col-12 p-0 input-group-sm">
                <label for="cat_search">Parent category</label>
                <input
                    type="text"
                    id="cat_search"
                    v-model="search"
...
export default {
    ...
    methods: {
        async applyFilters(){
            const {data} = await axios.get(`/admin/categories/list${this.search ? '?search=' + this.search : ''}`)
