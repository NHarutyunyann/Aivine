<template>
    ...
    <div class="liquid_product">
        <a href="/product-category/shutov-tesakani" class="liquid_product_title_box">
            <img src="/images/icons/radix-icons_lap-timer.svg" alt="" class="title_box_img img-soon" >
            <span>շուտով</span>
        ...
        <div class="liquid_product_images_box">
            <div @click="onClick(product)" class="liquid_image_box" v-for="product in products">
                ...
                <p v-if="product.regular_price" class="new_price">{{ priceFormatter(product.discount_price || product.regular_price, $store.getters.currency, $store.getters.currencyRates) }}</p>
            </div>
...
export default {
    ...
    methods: {
        async getProducts() {
            const {data} = await axios.get(`/get-products-by-type/shutov-tesakani`)
